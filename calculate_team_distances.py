...
from utils import GeocodedTeam, GeocodedLeague, LeagueTravelDistances, TeamTravelDistances, TravelDistances
...
def team_pair_distance(team1: GeocodedTeam, team2: GeocodedTeam) -> float:
    if (min(team1["name"], team2["name"]), max(team1["name"], team2["name"])) not in distance_cache:
        distance_cache[(min(team1["name"], team2["name"]), max(team1["name"], team2["name"]))] = distance(team1["latitude"], team1["longitude"], team2["latitude"], team2["longitude"])
...
def main():
    ...
    for json_file in sorted(geocoded_dir.glob("*.json")):
        ...
        valid_teams = [team for team in league_data["teams"] if "latitude" in team and "longitude" in team]
        for team in valid_teams:
    ...
    output: TravelDistances = {
        ...
        "summary": {
            ...
            "overall_avg_distance_km": round(
                sum(t["avg_distance_km"] for t in all_teams_data.values()) / len(all_teams_data), 2
            ) if all_teams_data else 0
        }
    }
    ...
    for i, league in enumerate(sorted_leagues[:10], 1):
        print(f"{i}. {league["league_name"]}: {league["avg_distance_km"]} km ({league["team_count"]} teams)")
    print()
