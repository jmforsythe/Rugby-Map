"""Per-season CI cache for English football pyramid SVG/PNG raster outputs.

Rebuild when geocoded pyramid data, tier_mappings, or :mod:`football.pyramid_image` change.
"""
...
from football.map_common import FOOTBALL_DIST

logger = logging.getLogger(__name__)

FOOTBALL_PYRAMID_RASTER_CACHE_ROOT = REPO_ROOT / "_football_pyramid_raster_cache"

_PYRAMID_CODE_PATHS: tuple[Path, ...] = (
    REPO_ROOT / "rugby" / "pyramid_image.py",
    REPO_ROOT / "football" / "pyramid_image.py",
    REPO_ROOT / "football" / "pyramid_parents.py",
    REPO_ROOT / "football" / "league_names.py",
    REPO_ROOT / "football" / "map_common.py",
)
...
def pyramid_raster_inputs_digest(season: str) -> str:
    """Stable digest of inputs that affect football pyramid diagrams for ``season``."""
    hasher = hashlib.sha256()
    for code_path in _PYRAMID_CODE_PATHS:
        if code_path.is_file():
            _hash_file(hasher, code_path)
    geocoded = REPO_ROOT / "data" / "football" / "geocoded_teams" / season / "pyramid"
    if geocoded.is_dir():
        for fp in sorted(geocoded.rglob("*")):
            if fp.is_file():
                _hash_file(hasher, fp)
    tier_mappings = REPO_ROOT / "data" / "football" / "tier_mappings" / f"{season}.json"
...
def pyramid_raster_cache_slot(season: str) -> Path:
    """Directory used with ``actions/cache`` path ``_football_pyramid_raster_cache/<season>``."""
    return FOOTBALL_PYRAMID_RASTER_CACHE_ROOT / season
...
def cache_is_valid(season: str, *, digest: str | None = None) -> bool:
    ...
    if "pyramid.svg" not in names:
        return False
    if "pyramid_Labels.svg" not in names:
        return False
...
def save_pyramid_raster_cache(season: str) -> int:
    ...
    logger.info(
        "Saved %d football pyramid artifact(s) for %s (digest %s) → %s",
        len(artifacts),
        season,
        digest,
        slot,
...
def restore_pyramid_raster_cache(season: str) -> int:
    ...
    for path in list_pyramid_artifacts(slot):
        ...
        restored += 1
    logger.info("Restored %d football pyramid artifact(s) for %s from cache", restored, season)
    return 0


def _cli() -> int:
    setup_logging()
    parser = argparse.ArgumentParser(description="Football pyramid raster CI cache helpers")
    sub = parser.add_subparsers(dest="command", required=True)

    digest_p = sub.add_parser("digest", help="Print inputs digest for a season")
    digest_p.add_argument("season")
