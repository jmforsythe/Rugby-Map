...
def generate_sitemap(tier_maps_dir: Path) -> str:
    """Walk tier_maps/ and produce a sitemap.xml string."""
    urls: list[str] = []

    for html_file in sorted(tier_maps_dir.rglob("index.html")):
        rel = html_file.relative_to(tier_maps_dir).parent.as_posix()
        path = f"/{quote(rel)}/" if rel != "." else "/"
        urls.append(path)

    for html_file in sorted((tier_maps_dir / "teams").glob("*.html")):
...
def main() -> None:
    ...
    sitemap = generate_sitemap(tier_maps_dir)
    sitemap_path = tier_maps_dir / "sitemap.xml"
    sitemap_path.write_text(sitemap, encoding="utf-8")
    url_count = sitemap.count("<url>")
    print(f"Created {sitemap_path} ({url_count} URLs)")

    robots = generate_robots()
    robots_path = tier_maps_dir / "robots.txt"
