...
def create_tier_maps(teams_by_tier: Dict[str, List[MapTeam]], tier_order: List[str], region_to_teams: RegionToTeams, itl_hierarchy: ITLHierarchy, output_dir: str = "tier_maps", show_debug: bool = True, season: str = "", team_travel_distances: Optional[TravelDistances] = None, ) -> None:
    ...
    for tier_num, tier in enumerate(tier_order):
        ...
        output_file = os.path.join(output_dir, f"{tier_name}{"/index.html" if IS_PRODUCTION else ".html"}")
        m.save(output_file)
        print(f"Saved {tier} map with {len(teams)} teams to: {output_file}")

def create_all_tiers_map(teams_by_tier: Dict[str, List[MapTeam]], tier_order: List[str], region_to_teams: RegionToTeams, itl_hierarchy: ITLHierarchy, output_dir: str = "tier_maps", output_name: str = f"All_Tiers", show_debug: bool = True, season: str = "", team_travel_distances: Optional[TravelDistances] = None) -> None:
    ...
    output_file = os.path.join(output_dir, f"{output_name}{"/index.html" if IS_PRODUCTION else ".html"}")
    m.save(output_file)
