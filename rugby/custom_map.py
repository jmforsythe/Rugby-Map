...
# Files written by this module; anything else under OUTPUT_DIR is removed on build.
# Boundary data lives at dist/shared/boundaries-custom.json (shared across rebuilds,
# not per-build under OUTPUT_DIR) — see _export_boundaries_custom.
OUTPUT_ASSETS = frozenset(
    {
        "teams.js",
        "distances.js",
        "bonus_imports.js",
    ...
    for season in seasons:
        ...
        for json_path in sorted(season_dir.rglob("*.json")):
            ...
            for team in league.get("teams", []):
                ...
                if td:
                    dist_league_name = td.get("league", "")
                    ld = dist_leagues.get(dist_league_name)
                    if dist_league_name and not league_name:
                        entry["lg"] = dist_league_name
...
def _export_boundaries_custom(
    ...
    """Write dist/shared/boundaries-custom.json with simplified England-only
    geometries and hierarchy (the custom map's own schema — named region dicts
    plus ward-level hierarchy for client-side Turf territory splitting, distinct
    from the GeoJSON FeatureCollections in dist/shared/boundaries.json used by
    the Folium tier maps). Written once to a shared location instead of
    per-build under OUTPUT_DIR, so it isn't duplicated as boundaries.js."""
...
def _build_page() -> None:
    ...
    if is_prod:
        ...
        seo_extra = (
            ...
            + breadcrumb_ld_script(
                [("Home", f"{BASE_URL}/"), ("Custom Map", page_url)],
                indent="    ",
            )
            + "\n"
        )
    ...
    home_link_html = (
        f'      <a class="app-header__home" href="{html_escape(home_href)}">← Home</a>\n'
    )
...
def _prune_stale_outputs() -> None:
    """Remove obsolete files from ``dist/custom-map/`` (e.g. legacy projected.js)."""
    if not OUTPUT_DIR.is_dir():
        return
    for path in OUTPUT_DIR.iterdir():
        if path.is_file() and path.name not in OUTPUT_ASSETS:
            path.unlink()
            logger.info("Removed stale custom-map output %s", path.name)
...
def main() -> None:
    ...
    # Write dist/shared/boundaries-custom.json
    if has_boundaries:
        _export_boundaries_custom(
            itl1_regions,
            itl2_regions,
            itl3_regions,
            lad_regions,
